<?php
    ...
    class EC2InstancePrices {
        ...
        private $EC2_INSTANCE_TYPES = array(
            "t1.micro",
            "m1.small",
            "m1.medium",
            "m1.large",
        ...
        private $EC2_OS_TYPES = array(
            "linux",
            "mswin"
        );

        private $JSON_NAME_TO_EC2_REGIONS_API = array(
            "us-east" => "us-east-1",
            "us-east-1" => "us-east-1",
            "us-west" => "us-west-1",
            "us-west-1" => "us-west-1",
        ...
        private $EC2_REGIONS_API_TO_JSON_NAME = array(
            "us-east-1" => "us-east",
            "us-west-1" => "us-west",
            "us-west-2" => "us-west-2",
        ...
        private $INSTANCE_TYPE_MAPPING = array(
            "stdODI" => "m1",
            "uODI" => "t1",
            "hiMemODI" => "m2",
            "hiCPUODI" => "c1",
        ...
        private $INSTANCE_SIZE_MAPPING = array(
            "u" => "micro",
            "sm" => "small",
            "med" => "medium",
            "lg" => "large",
        ...
        function get_ec2_reserved_instances_prices($filter_region = NULL, $filter_instance_type = NULL, $filter_os_type = NULL) {
            ...
            $urls = array(
                $this -> INSTANCES_RESERVED_LIGHT_UTILIZATION_LINUX_URL,
                $this -> INSTANCES_RESERVED_LIGHT_UTILIZATION_WINDOWS_URL,
                $this -> INSTNACES_RESERVED_MEDIUM_UTILIZATION_LINUX_URL,
                $this -> INSTANCES_RESERVED_MEDIUM_UTILIZATION_WINDOWS_URL,
                $this -> INSTANCES_RESERVED_HEAVY_UTILIZATION_LINUX_URL,
                $this -> INSTANCES_RESERVED_HEAVY_UTILIZATION_WINDOWS_URL
            );
                ...
                if ($data["config"] && $data["config"]["regions"]) {
                    foreach ($data["config"]["regions"] as $r) {
                        ...
                        if (array_key_exists($region_name, $result_regions_index)) {
                            ...
                            $result_indices[$region_name] = array_push($result["regions"], array(
                                "region" => $region_name,
                        ...
                        if (array_key_exists("instanceTypes", $r)) {
                            foreach ($r["instanceTypes"] as $it) {
                                $instance_type = isset($this -> INSTANCE_TYPE_MAPPING[$it["type"]]) ? $this -> INSTANCE_TYPE_MAPPING[$it["type"]] : '';
                                if (array_key_exists("sizes", $it)) {
                                    foreach ($it["sizes"] as $s) {
                                        if (strpos($s["size"], '.') !== false) {
                                            // If the whole instance type is in it
                                            $_type = $s["size"];
                                        } else if (isset($this -> INSTANCE_SIZE_MAPPING[$s["size"]])) {
                                        ...
                                        // var_dump($currency, $s["valueColumns"]);
                                        ...
                                        array_push($result["regions"][$result_indices[$region_name]]["instanceTypes"], array(
                                            "type" => $_type,
                                            "os" => $os_type,
                ...
                foreach ($data["config"]["regions"] as $r) {
                    if ($r["region"]) {
                        ...
                        if (array_key_exists("instanceTypes", $r)) {
                                ...
                                if (array_key_exists("sizes", $it)) {
                                    foreach ($it["sizes"] as $s) {
                                        ...
                                        foreach ($s["valueColumns"] as $price_data) {
                                            ...
                                            array_push($instance_types, array(
                                                "type" => $_type,
                                                "os" => $price_data["name"],
                                                "price" => $price
                                            ));
                            ...
                            array_push($result["regions"], array(
                                "region" => $region_name,
                                "instanceTypes" => $instance_types
                            ));
                        }
