<!--
<p class="text-center">
	<a title="Facebook" href="{{URL::to('user/social')}}?provider=Facebook" class="social-facebook "><i class="fa fa-facebook-square fa-4x social-login-icons"></i></a>
	<a title="Google" href="{{URL::to('user/social')}}?provider=Google" class="social-google "><i class="fa fa-google-plus-square fa-4x social-login-icons"></i></a>
</p>
-->
<p class="text-center">
	<a title="Github" href="{{URL::to('user/social')}}?provider=GitHub" class="social-git"><i class="fa fa-github-square fa-4x social-login-icons"></i></a>
	<a title="LinkedIn" href="{{URL::to('user/social')}}?provider=LinkedIn" class="social-linkedin "><i class="fa fa-linkedin-square fa-4x social-login-icons"></i></a>
</p>
